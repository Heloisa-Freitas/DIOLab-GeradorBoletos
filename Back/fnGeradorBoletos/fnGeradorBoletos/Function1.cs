    ...
    public class GeradorCodigoBarras
    {
        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post")] HttpRequest req)
        {
            try
            {
                ...
                string valor = Convert.ToString(data?.valor);
                string dataVencimento = Convert.ToString(data?.dataVencimento);
                ...
                if (skImage == null)
                {
                    _logger.LogError("⚠️ Erro: skImage retornou null para o código: " + barcodeData);
                    return new BadRequestObjectResult("Erro ao gerar a imagem do código de barras.");
                }
                ...
                using (var encodeData = skImage.Encode(SkiaSharp.SKEncodedImageFormat.Png, 100))
                {
                    ...
                    _logger.LogInformation($"Imagem gerada. Bytes: {imageBytes.Length}, Base64 OK: {!string.IsNullOrEmpty(base64String)}");
